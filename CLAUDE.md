...
- **Database (`db/`):** Local homelab Postgres (`homelab` DB, `drophunter` schema) via psycopg2 in `db/client.py`. Game tables: `games`, `price_history`, `notifications_log`. Watch tables: `watches`, `watch_price_history`, `watch_notifications_log`. Chat memory (`chat_messages`, `chat_summary`) and the access allowlist (`allowed_users`). All tables are schema-qualified (`drophunter.*`). `games`/`watches` carry a `user_id` owner column (composite-unique per user: `(user_id, itad_id)`, `(user_id, swisstimehouse_url)`). Schema DDL lives in `db/migrations/001_drophunter_schema.sql`. Supabase is a cold backup only — synced every 3 days by `cron/supabase_backup.py`.
...
- **Environment variables:** `LOCAL_DB_URL` (primary DB), `SUPABASE_URL`/`SUPABASE_KEY` (backup only), `ITAD_API_KEY`, `GROQ_API_KEY`, `GEMINI_API_KEY`, `AI_PROVIDER`, `DISCORD_BOT_TOKEN`, `OWNER_ID`, `LANGFUSE_*` — use `.env` locally; GitHub Actions secrets in CI. (`DISCORD_CHANNEL_ID`/`DISCORD_WEBHOOK_URL` are no longer used — DM-only.)
