...
- **Scheduler (`cron/`):** `cron/price_check.py` runs the price sweep — `process_game` sweeps tracked games (compares against historical lows) and `process_watch` sweeps tracked watches (compares the lowest available price against the watch's required target). Both fire Discord webhook alerts with AI commentary. The entrypoint accepts `--games`/`--watches` flags (no flag = both). Sweeps run every 12h via two GitHub Actions workflows: `price_check.yml` (`--games`, GitHub-hosted runner) and `watch_check.yml` (`--watches`, **self-hosted runner on the Mac mini** — Swiss Time House's Cloudflare blocks GitHub's datacenter IPs, so the watch sweep must run from a residential IP).
